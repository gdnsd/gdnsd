...
files which gdnsd interacts with live within that chroot directory.

In the system case, gdnsd will create its own basic configuration and run
directories if they're missing on startup, but will not recursively
create parent directories (e.g. it will create F</var/run/gdnsd> if missing,
but will fail if F</var/run> itself is missing.  In the same manner
it will create F</etc/gdnsd> (and other gdnsd-specific subdirectories
thereof), but will fail if F</etc> itself is missing.
...
C<example.com> present in the zones directory) are handled by
