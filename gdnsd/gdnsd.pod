...
=head1 SYNOPSIS
  ...
  Usage: gdnsd [-fsSD] [-c /path/to/configfile] <action>
    ...
    -c - Main config file, defaults to e.g. /etc/gdnsd/config
  Actions:
...
There is a single primary configuration file. The default location is
a compile-time constant, usually C<${sysconfdir}/gdnsd/config>, where
C<$sysconfidir> is generally e.g. F</etc>.
...
location as the main config file.
