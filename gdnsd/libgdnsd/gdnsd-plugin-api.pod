...
A C<service_type> is meant to be re-used to monitor the same service at
...
instance, such as the actions C<stop>, C<restart>, or C<lpe_on>.  This is
...
here will be passed back to your code for dynamic resolution.  If your DYNC
plugin supports variable origins (e.g. the same resource name can be re-used
in multiple zonefiles, and prepends some standard domainname fragment to
origin in effect for the given RR), it is important that you at least validate
that you can construct a legal domainname (length limits) from the given
origin, resource name, and your own config at this time.  You may wish
to return unique resource numbers for each unique combination of
origin+resname and cache pre-built return values as well.
...
gdnsd provides an integer type C<satom_t> and functions C<satom_set>,
C<satom_inc>, and C<satom_get> which are useful for lock-free multi-threaded
stats counters.  The basic idea is that you allocate a C<satom_t> per-thread,
modify it using C<satom_inc> from within calls like resolve_dynaddr, and then
read and coalesce the data of all threads from the primary thread using
C<satom_get>.

This is the same mechanism the core gdnsd code uses for basic DNS stats
counters.  This mechanism is not fully atomic.  You should never use the write
operations (set and inc) from multiple threads on a single satom_t.  You can,
however, read a satom_t from many threads in parallel without locking, so long
as only one thread performs all writing (again, without locking).  If you chose
to use this mechanism, you need to also be wary of false sharing that may occur
if you allocate several threads' satom_t structures in the same page of memory.

Since most C libraries now have thread-aware malloc(), it would be best to
malloc the per-thread satom_t's from the threads themselves during
C<plugin_foo_iothread_init>.
...
This version existed in a long-standing dev series, but never a release build.
...
The trivial plugins C<null>, C<reflect>, C<static>, C<simplefo>, and C<multifo>
in the F<plugins/trivial> subdirectory of the core gdnsd source code.

The C<georeg> plugin, available in source form separately at the
gdnsd download site.

The C<skel> plugin, a baseline autoconf project setup for a new
plugin, available in the gdnsd source repository.
