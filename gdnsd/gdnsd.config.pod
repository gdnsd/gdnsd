...
=item B<zreload_disable>

Boolean, default false.  If set to true, any mechanism for dynamically
updating zone data at runtime will be disabled, and the server daemon
will have to go through a complete restart cycle to pick up any
changes.

=item B<zreload_scan_interval>

Integer seconds, default 31, min 10, max 600.  For zone data sources
which support loading new data at runtime (such as the default and
only rfc1035 zonefiles source, for now), this represents the interval
for the code to check for new zone data updates.

Note that depending on the source type and the platform, there could be
efficient mechanisms for receiving notifications faster than this (e.g.
C<inotify()> on modern Linux), in which case the scan interval might only
apply to some rare corner cases when the more efficient mechanism fails.

=item B<zreload_quiesce_period>

Integer seconds, default 5, min 3, max 60.  When a zone data change
is detected (via periodic scanning controlled by B<reload_scan_interval>,
or directly via e.g. C<inotify()>), the change will remain pending
for this period of time to ensure that rapid-fire updates to the source
are coalesced into a single, consistent data reload.

If the source changes while the quiesce period is counting down, the
...
data) will the new data be loaded for runtime queries.
