...
on C<0.0.0.0> and C<::> (if IPv6 support is detected).
...
If the listen option isn't specified at all, for historical compatibility reasons
the current default is C<scan>.  However, this default is expected to change to
C<any> in a future version, so you should specify C<scan> explicitly if you require
this behavior.
...
Integer, default 10, min 1, max 65535.  Number of monitoring requests
which must succeed with no intervening failures to transition a given
IP for this resource from the DANGER state to the UP state.

=item B<down_thresh>

Integer, default 10, min 2, max 65535.  Number of monitoring requests
...
(this includes the initial failure that moves from UP to DANGER).
