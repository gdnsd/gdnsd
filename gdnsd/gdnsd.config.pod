...
Note in B<listen> above that the default IP address for listen
specifications results in creating two listening sockets, one
each for IPv4 and IPv6.  This limit applies separately to each.
...
Integer, min 4096, max 1048576.  If set, this value will be used to set the
C<SO_SNDBUF> socket option on the UDP listening socket(s).  Most users do not
need to tune this value.  If left unset, the code only takes a somewhat
heuristic approach, trying to raise the value only if the OS-supplied default
seems too low.
...
If explicitly set, gdnsd will attempt <setpriority()>
...
containing actual response data.  Resolvers in general do not seem to
ever really require this data to resolve any ambiguity, and Caches
will generally refresh the NS data when necessary.  Leaving this
option in its default state results in smaller response packets
and faster response packet generation in many common cases.  This
is similar in nature to (but not exactly like) BIND's
"minimal-responses" option, except that we default to the minimal mode.
...
RRsets in the answer section.  Additionally, "glue" address information
which may be necessary for the namservers of a delegation is *always*
included.

This option never directly affects the generation of Additional records,
although when NS records are omitted, obviously that indirectly removes
any related Additional records which weren't otherwise referenced in
the Answer section.

As far as additional records go, gdnsd's general policy is to include
Additional records for any right-hand-side name mentioned in the Answer
or Authority section if that name is in the same zone as the question
asked.  Additionally, any C<A> -query will result in C<AAAA> Additional
records being added and vice-versa, if they exist.
...
for this request, the response packet will contain a dangling CNAME, and a packet
error will be logged if packet error logging has been turned on.

Note that this is also the only thing preventing infinite CNAME loops.
...
=item B<simplefo> - simple primary->secondary failover of monitored addresses

=item B<multifo> - all-active failover of monitored round-robin address groups

=item B<weighted> - weighted-round-robin responses with a variety of behavioral flavors, for both monitored addresses and CNAMEs

=item B<metafo> - static-ordered address(-group) meta-failover between 'datacenters', which are resources defined in terms of other plugins

=item B<geoip> - combines metafo's functionality with MaxMind GeoIP databases to select different datacenter address(-group) preference/failover orderings for different clients based on approximate geographic location.  Supports geographically-differentiated CNAME resolution as well.
...
Copyright (c) 2010 Brandon L Black <blblack@gmail.com>
