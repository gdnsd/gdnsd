...
configuration file.  It always lives at the pathname
F</etc/config> within the data root directory for a
given execution of the gdnsd binary (the C<-d> argument,
which in turn defaults to F<PREFIX/gdnsd>).  For example,
when a default installation to F</usr/local> is invoked
with the default data root directory, the config file
is loaded from C</usr/local/gdnsd/etc/config>.
...
if you're happy with an all-default configuration.
...
At daemon start, a heuristic test will determine whether
we can use a faster 0.01s or the default 1.02s as
a basic sane minimum for the zones filesystem, and this
config setting will be adjusted upwards to the detected
...
accuracy, but also waiting out additional intentional actions
