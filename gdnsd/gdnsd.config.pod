...
Integer seconds, default 5, min 3, max 60.

When a change is detected, this value is used
as a quiescence timer before actually processing the change.

If the source changes while the quiescence timer is counting down, the
counter will be reset.  Only after the zone data source has stabilized
for the full period of this timeout (plus the time to actually load the
data) will the new data be loaded and presented for runtime queries.

When not using C<inotify()>, this quiesce period applies to all
detected changes.  In the C<inotify()> case, however, certain types
of change activity which generally result from consistent atomic
operations (e.g. mv/rename new zone data into place, unlink zonefile)
will quiesce in 1.02 seconds instead of waiting for e.g. further
write traffic like the general case.
