...
It is also important to note that, currently, CNAMEs do not have any
associated status information and never "fail", and therefore it only
makes sense to use one in the final datacenter of a resource as a
final fallback when all address-based datacenters are down.
