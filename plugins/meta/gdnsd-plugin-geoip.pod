...
String, filename, required.  This is the filename of one of the supported MaxMind GeoIP database types.  It is considered relative to the F</etc/geoip/> subdirectory of the current daemon invocation's data root directory (C<-d> argument), which in turn defaults to F<PREFIX/gdnsd/>.  It will be reloaded at runtime (without any singificant query interruptions) if a change to the database file is detected.

=head2 C<geoip_db_v4_overlay = GeoIP.dat>

String, pathname, optional.  This specifies an optional IPv4-level GeoIP database to overwrite the IPv4 sub-space of the IPv6 data loaded from C<geoip_db>.  It must be a V4-format database, and C<geoip_db> must be a V6-format database.  When runtime reloads are performed, if either database changes, both will be reloaded.  In all other respects, it is similar to C<geoip_db>.
