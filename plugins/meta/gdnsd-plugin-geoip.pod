...
This filename is considered relative to the F</etc/geoip/> subdirectory of the current daemon invocation's data root directory (C<-d> argument), which in turn defaults to F<PREFIX/gdnsd/>.  Currently, it is not tracked for runtime reloads on file content changes.
...
String, filename, optional.  This is the filename of one of the supported MaxMind GeoIP database types.  It is considered relative to the F</etc/geoip/> subdirectory of the current daemon invocation's data root directory (C<-d> argument), which in turn defaults to F<PREFIX/gdnsd/>.  It will be reloaded at runtime (without any significant query interruptions) if a change to the database file is detected.
...
String pathname, optional.  A variant of the above, but the contents of the key-value hash are loaded from the named external file (which is relative to and should reside within the F</etc/geoip/> subdirectory, just like e.g. geoip_db).  This makes life easier for external tools and scripts generating large sets of nets entries (e.g. from BGP data).  The file will be monitored for changes and reloaded at runtime much like the GeoIP databases.
