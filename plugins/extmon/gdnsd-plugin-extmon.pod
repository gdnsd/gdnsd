...
binary F<gdnsd_extmon_helper> in the installation C<$libexec/gdnsd>
directory for gdnsd.  If for some reason it can't do so,
...
failure is triggered for each resource.
...
found, the IP address of the resource being monitored will be substituted
in its place.
