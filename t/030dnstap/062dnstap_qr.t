#
# This tests dnstap queries and responses logging
#
...
if(!$child_pid){
    ...
    exec("dnstap -u $out_dir/062capture.tap > $out_dir/062capture.out; rm -f $out_dir/062capture.*");
} else {
    print "Hello from parent $child_pid \n";

    BEGIN { use_ok("_GDT") or diag("Test suite broken (no _GDT)"); }

    my $pid = _GDT->test_spawn_daemon('etc_qr');
    ...
    #sleep(3);
    my $dnstap_output = `cat $out_dir/062capture.out`;
