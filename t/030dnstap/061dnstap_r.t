#
# This tests dnstap only responses logging
#
...
if(!$child_pid){
    ...
    exec("dnstap -u $out_dir/061capture.tap > $out_dir/061capture.out; rm -f $out_dir/061capture.*");
} else {
    print "Hello from parent $child_pid \n";

    BEGIN { use_ok("_GDT") or diag("Test suite broken (no _GDT)"); }

    my $pid = _GDT->test_spawn_daemon('etc_r');
    ...
    my $dnstap_output = `cat $out_dir/061capture.out`;

    my @expected = ("AR 127.0.0.1 UDP 49b \"ns1.example.com.\" IN A",
