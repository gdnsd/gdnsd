#
# This tests dnstap disabled option
#
...
if(!$child_pid){
    ...
    exec("dnstap -u $out_dir/059capture.tap > $out_dir/059capture.out; rm -f $out_dir/059capture.*");
} else {
    print "Hello from parent $child_pid \n";

    BEGIN { use_ok("_GDT") or diag("Test suite broken (no _GDT)"); }

    my $pid = _GDT->test_spawn_daemon('etc_n');
    ...
    my $dnstap_output = `cat $out_dir/059capture.out`;
    my @expected = ();
