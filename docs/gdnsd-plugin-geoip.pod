...
of the direcotry containing the main daemon's configuration file.
The default would be F<@GDNSD_DEFPATH_CONFIG_DIR@/geoip> if using the
default config file path of F<@GDNSD_DEFPATH_CONFIG_FILE@>.  You can
load from other locations by specifying absolute file paths.
