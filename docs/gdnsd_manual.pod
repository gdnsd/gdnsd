...
user named C<gdnsd> (configurable) and chroot into the default data root
directory (or a non-default data root directory given by C<-d> on the
commandline) before answering any network queries.
