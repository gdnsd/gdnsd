...
The scaling of UDP DNS was looked into extensively on Linux.  Adding more threads
...
Request was for a non-existant domainname.  In other words,
a name the daemon is authoritative for, but which does not exist in the
database.

=item notimp

Requested service not implemented by this daemon, such as zone transfer requests.

=item badvers

Request had an EDNS OPT RR with a version higher than
zero, which this daemon does not support (at the time of this writing,
such a version doesn't even exist).

=item dropped

Request was so horribly malformed that we didn't even
...
Request was from an IPv6 client.  This one isn't RCODE based,
and is orthogonal to all other counts above.

=item edns

Request contained an EDNS OPT-RR.  Not RCODE-based, so
again orthogonal to the RCODE-based totals above.  Includes the
ones that generated badvers RCODEs.

=item edns_client_subnet

Subset of the above which specified the
...
Total count of UDP requests received and passed on
to the core DNS request handling code (this is synthesized by summing
all of the RCODE-based stat counters above for the UDP threads).

=item udp_recvfail

Count of UDP recvmsg() errors, where the OS indicated
that something bad happened on receive.  Obviously, we don't even get these
requests, so they can't be processed and replied to.

=item udp_sendfail

Count of UDP C<sendmsg()> errors, which almost definitely
resulted in dropped responses from the client's point of view.

Note that a common cause for these I've seen on Linux is requests
with a source port of zero.  For some reason Linux will pass these
to us on the input side, but then the C<sendmsg()> call to send a
response back to port zero immediately fails due to the bad port number.

=item udp_tc

Non-EDNS (traditional 512-byte) UDP responses that were
truncated with the TC bit set.

=item udp_edns_big

Subset of udp_edns where the response was greater
than 512 bytes (in other words, EDNS actually did something for you
size-wise)

=item udp_edns_tc

Subset of udp_edns where the response was truncated
and the TC bit set, meaning that the client's specified edns buffer size
was too small for the data requested.
...
Total count of TCP requests (again, synthesized by summing the RCODE-based
stats for only TCP threads).

=item tcp_recvfail

Count of abnormal failures in recv() on a DNS TCP
...
are uintptr_t, which helps with rollover on 64-bit machines.
...
elided to make room for other records.  Nameserver address RRs for
...
Also, by default, unnecessary lists of NS records in the Authority section are
...
user named C<gdnsd> (configurable).  It will optionally also chroot into
a chroot directory specified via the C<-d> argument (or defaulted at build time),
but the default default is to use system pathnames and not do chroot.

If any security-related operation fails, the daemon will fail to start itself
and abort with a log message indicating the problem.
