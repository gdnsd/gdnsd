...
configuration file.  The default path for the config file
is F<@GDNSD_DEFPATH_CONFIG_FILE@>, but this can be overridden
by the C<-c> commandline option.
...
There are also two special singalur string values: C<any> and C<scan>.

If set to C<any>, the daemon will listen on the C<dns_port> port (default 53)
on C<0.0.0.0> and C<::> (if IPv6 support is detected).  This is the default
if no C<listen> option is provided.
...
ones closes or times out.  Note that sockets map 1:1 to threads, and thus
the total client limit for connecting to a given address would be
C<tcp_clients_per_thread * tcp_threads> for a given address.
...
on initial startup or checkconf, this results in a fatal error (failure
to start, or non-zero exit code, respectively).  During a runtime
zone data reload, any existing good copy of the zone would continue
to be served until the error is corrected in the source.
...
There are four service_type plugins included with gdnsd:

C<tcp_connect> checks basic TCP connectivity to a given address.
At this time, it only supports the monitoring of address (as opposed to
CNAME) resources.  It has just one plugin-specific parameter:
...
C<http_status> performs an HTTP request to a given address (as with
C<tcp_connect>, CNAMEs are not yet supported), and has the following
...
String, default C</>.  This is the URL path that should be used when
...
C<extmon> periodically executes external commandline programs to poll
for the status of a given address or CNAME, and is documented separately
in L<gdnsd-plugin-extmon(8)>.

C<extfile> periodically reads the contents of a file (in the same basic
C<vscf> syntax as the main config file) to pick up status values set
there by some other program for address or CNAME resources, and is also
documented separately in L<gdnsd-plugin-extfile(8)>.
