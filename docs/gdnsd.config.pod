...
sending the daemon a C<SIGUSR1> (or executing the "reload" command /
initscript action, which sends SIGUSR1 for you).
...
Integer, default 1280, min 512, max 64000.  This is the maximum size
of a UDP edns response to a client, acting as a cap on the edns
buffer size advertised by the client in its request.  The default value
1280 was chosen as it's very likely that such a size will traverse
the public Internet un-fragmented (it is the IPv6 Minimum MTU value
for similar reasons), and most reasonable DNS datasets result in
responses significantly smaller than this.  The option obviously has
no pragmatic effect if you do not have large response datasets in your
