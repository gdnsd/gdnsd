...
With the C<-f> option, starts gdnsd in the foreground with logs
mirrored to both stderr and syslog.  Useful for debugging and
testing.
...
at runtime for changes, and any overriden state found is applied quickly.
