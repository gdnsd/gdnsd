...
=head1 SYNOPSIS
  ...
  Usage: gdnsd [-fsSD] [-c @GDNSDDEF_CONFIG_DIR@] <action>
...
Sets foreground mode for start/restart -like actions.  The daemon will
remain attached to the controlling terminal and will not fork-and-terminate
to free up the invoking shell.  It will, however, participate in pidfile
locking with other (e.g. background) instances of itself using the same
configuration.
...
C<SO_REUSEPORT> became available in Linux kernels starting with kernel
...
The zones directory is the subdirectory named C<zones> in the
configuration directory.

All files in the zones directory are considered zone files.
In general there should be exactly one file
...
Important file/directory paths for the core daemon code:

=over 4

=item F<@GDNSD_DEFPATH_CONFIG@>

Default configuration directory, unless overriden via C<-c>.  The
...
for administrative state-overrides on monitored resouces (etc).  See
the entry for C<state_dir> in the L<gdnsd.config(5)> manpage for more
...
not run.  The only current case is F<gdnsd_extmon_helper> for C<plugin_extmon>,
and the path for this can be overridden in that plugin's configuration,
documented in L<gdnsd-plugin-extmon(8)>.
