...
* Flags:
  ...
  * Added: `-T` - allows `start` or `daemonize` to take over operations from another running instance in a smooth (downtime-less, loss-free) way.  This is what's used internally when the daemon spawns its own replacement when commanded by `gdnsdctl replace`.  Without `-T`, if another daemon instance were already running, `start` or `daemonize` would complain and exit.
